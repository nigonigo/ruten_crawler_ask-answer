...
ruten_url <- 'http://goods.ruten.com.tw/item/show?21408018125710#history&p=1'

session <- GET((ruten_url),add_headers('user-agent' = 'uagent'))
...
test2 <- read_html(session) %>% 
  
  html_nodes(".rt-text-parentesis")
